...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
	...
	<script language="javascript" type="text/javascript" id="\">    
        function handlecosteffective(e, myradio) {
            var selectedvalue = $('#<%= rdoCostEffective.ClientID %> input:checked').val()
     //alert(selectedvalue);
            var inputbox = $("#<%=txtDesignNotCostEffective.Clientid%>");
            if (selectedvalue === 'No') {
                inputbox.prop('disabled', '')
            }
            else {
        ...
        function handleashraecert(myRadio) {
            var selectedvalue = $('#<%= rdoCertStatus.ClientID %> input:checked').val()
           // alert(selectedvalue);
           var inputbox = $("#<%=txtDateCertifed.Clientid%>");
            var dd = $("#<%=ddLEEDCertLevelAcheived.ClientID%>");
    ...
    <script language="javascript" type="text/javascript" id="\">
        ...
        txtbox.addEventListener("input", formatNumber);
        txtbox.addEventListener("change", formatNumber);
        txtbox.addEventListener("paste", formatNumber);
        txtbox.addEventListener("focusout", formatNumber);
     ...
     // alert(selectedvalue);
      if (selectedvalue === 'No'  || selectedvalue === 'Unknown') {
          ...
          var dd = $("#<%=ddCurrProjected.ClientID%>");
          dd.prop('disabled', 'true');
          $("#<%=rdoLeedWaiver.ClientID %>").find('input').prop('disabled', 'true');
          $("#<%=rdoCertStatus.ClientID %>").find('input').prop('disabled', 'true');
          //$("#<%=txtDateCertifed.ClientID %>").find('input').prop('disabled', 'true');
          var inputBox = $("#<%=txtDateCertifed.ClientID%>");
          inputBox.prop('disabled', true);
          //$("#<%=ddLEEDCertLevelAcheived.ClientID %>").find('input').prop('disabled', 'true');
          var inputBox = $("#<%=ddLEEDCertLevelAcheived.ClientID%>");
          inputBox.prop('disabled', true);
      }
      else {
          ...
          //$("#<%=txtDateCertifed.ClientID %>").find('input').prop('disabled', '');
          var inputBox = $("#<%=txtDateCertifed.ClientID%>");
          inputBox.prop('disabled', '');
          //$("#<%=ddLEEDCertLevelAcheived.ClientID %>").find('input').prop('disabled', '');
                var inputBox = $("#<%=ddLEEDCertLevelAcheived.ClientID%>");
                inputBox.prop('disabled', '');
            }
        }
        ...
        function myconfirm() {														
            ...
            var x = confirm("Leaving build. Do you want to save");
            return x;
        ...
        function handleperform30PCBetter(e, myradio) {
            var selectedvalue = $('#<%= rdoASHRAE30PerCent.ClientID %> input:checked').val()
           // alert('handleperform30PCBetter'); handleperform30PCBetter
       // alert(selectedvalue);
            if (selectedvalue === "Yes" || selectedvalue === 'Unknown') {
                var inputbox = $("#<%=txtexceedsbaseline.clientid%>");
                inputbox.prop('disabled', '')
                var inputbox = $("#<%= txtNot30PerCentBetterExplanation.Clientid%>");
                inputbox.prop('disabled', 'true')
                $("#<%=rdocosteffective.ClientID %>").find('input').prop('disabled', 'true');
                var inputbox1 = $("#<%=txtDesignNotCostEffective.Clientid%>");
                ...
                var inputbox = $("#<%= txtNot30PerCentBetterExplanation.Clientid%>");
                inputbox.prop('disabled', '')
                $("#<%=rdocosteffective.ClientID %>").find('input').prop('disabled', '');
                var inputbox1 = $("#<%=txtDesignNotCostEffective.Clientid%>");
                inputbox1.prop('disabled','')
            }
...
function txtintcomma(e,txtbox) {
    //alert(e.code);
    if (!e.code.includes('Digit')/* && !e.code.includes('Comma')*/) {
        var len = txtbox.value.length;
        var final = txtbox.value.substring(0, len - 1);
        if (!e.code.includes('Shift') && !e.code.includes('Enter') && !e.code.includes('Arrow') && !e.code.includes('Backspace') && !e.code.includes('Control')
            && !e.code.includes('Shift') && !e.code.includes('Alt') && !e.code.includes('Meta')
            && !e.code.includes('Caps'))
            txtbox.value = final;
    }
    else {
        ...
        const number = parseInt(String(word));
         const formatted = number.toLocaleString();
         txtbox.value = formatted;
    }
}

        function handleleedregisteronline(e) {
        ...
        function handlenetzerorequirement() {
          
            var selectedvalue = $('#<%=rdoNetZApplicable.ClientID %> input:checked').val()
           // alert(selectedvalue);
            //alert(selectedvalue);
            if (selectedvalue === "No") {
                var inputbox = $("#<%=txtNetZeroApplicableNoExplain.Clientid%>");
             inputbox.prop('disabled', '')
  }
  else {
              var inputbox = $("#<%= txtNetZeroApplicableNoExplain.Clientid%>");
        ...
        //Neil Newton
        function handlerdoProjectedLeedSilverClick(myRadio) {
        ...
        //alert(document.getElementByID("rdoProjectedLeedSilver").checked);
            //var reasons = document.getElementsByName("<%=rdoProjectedLeedSilver.UniqueID%>");
                ...
                if (selectedvalue != "No") {
                       // alert('NoPicked');
                        $("#<%=rdoLeedWaiver.ClientID %>").find('input').prop('disabled', 'true');
                //$("#<%=rdoLeedWaiver.ClientID %>").find('input').prop('forecolor', 'red ');
                document.getElementById("rdoLeedWaiver.ClientID").style.backgroundColor = red;
                var elementRef = document.getElementById('<%= rdoProjectedLeedSilver.ClientID %>');
                //elementRef.backgroundColor = red;
                //rdoleedwaiverspan.backgroundColor = red;
                    ...
                    <!-- Sustainability tab Start Neil ....-->
                     <cc1:TabPanel ID="tpPanelSustainability" runat="server">
                        <HeaderTemplate>Sustainability</HeaderTemplate>
                         ...
                         <span style="color:red;">* </span>&nbsp;&nbsp;<span style="font-size:large;">Number of buildings in this phase</span>&nbsp;&nbsp;&nbsp;&nbsp;
                                        <span style="font-size:large;width:400px;">
                                            <asp:DropDownList ID="ddSustCoord" OnSelectedIndexChanged_="ddSustCoord_OnSelectedIndexChanged" AutoPostBack="True" runat="server">
                                                ...
                                                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;  &nbsp;  &nbsp; &nbsp;&nbsp; &nbsp;  &nbsp;Choose a sustainability planner&nbsp;&nbsp;
                                                    <asp:DropDownList ID="ddChooseSustCoord" runat="server"    AutoPostBack="True" >
                                                               <asp:ListItem Value="0">Please Select</asp:ListItem>
                                                                <asp:ListItem Value="1">Planner 1</asp:ListItem>
                                                                <asp:ListItem Value="2">Planner 2</asp:ListItem>
                                                                <asp:ListItem Value="3">Planner3</asp:ListItem>
                                                                <asp:ListItem Value="4">Planner 4</asp:ListItem>
                                                        </asp:DropDownList>
                                        </span><br />

                                         <asp:Button ID="BtnSavePhase" runat="server" OnClick="BtnSavePhase_Click" Text="SAVE" Height="40px" Width="60px" Font-Bold="True" />                               
						</asp:Panel>	
                            ...
                            <asp:Panel ID="PnlGeneralInfo" BackColor="#E6ECFF" BorderStyle="Solid" runat="server">
                                ...
                                <span style="font-size:x-large;float:left;width:400px">General Information</span>&nbsp;<br /><br />
                                 ...
                                 <span style="float:right;margin-top:10px;margin-right:5px">
                                    <asp:textbox  ID="txtSqFtConst" Width="300px" maxlength="11"   runat="server" onkeyup="txtintcomma(event,this);" ></asp:textbox>
                                 </span><br />

                                 <br /> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style="float:left;vertical-align:auto;margin-left:5px;font-size:large;">Square Footage of renovation work</span>
                                 <span style="float:right;margin-top:10px;margin-right:5px">
                                    <asp:textbox  ID="txtSqFtRenovation" Width="300px"  maxlength="11"  runat="server" onkeyup="txtintcomma(event,this);"></asp:textbox>
                                 </span><br />
                             ...
                             <asp:Panel ID="PnlLeeds" BorderStyle="Solid" runat="server">
                               ...
                               <span style="font-size:large;float:left;margin-leftrovide the LEED registration number.
5px">(For all buildings entering design on/after August 3, 2021)</span><br /><br />
                                ...
                                <span style="font-size:medium;float:right;margin-right:5px" >
                                        ...
                                        <asp:RadioButtonList ID="rdoLEEDRequired" runat="server"  RepeatDirection="Horizontal" onclick=" handleleedrequired(this);" AutoPostBack="false">
                                 ...
                                 <span style="font-size:medium;float:right;margin-right:5px">
                                     <asp:RadioButtonList ID="rdRegisteredOnline" runat="server"  RepeatDirection="Horizontal" AutoPostBack="false" onclick="handleleedregisteronline(event,this);"  >
                                            <asp:ListItem Value="Yes">Yes</asp:ListItem>
                                            <asp:ListItem Value="No">No</asp:ListItem>
                                            <asp:ListItem Value="Unknown">Unknown</asp:ListItem>
                                        </asp:RadioButtonList>                                    
                                 ...
                                 <span style="float:right;vertical-align:auto;margin-right:5px">
                                    <asp:textbox  ID="txtLeedRegistration"  Width="300px"  Maxlength="15"  runat="server"  onkeyup="txtintcomma(event,this);" ></asp:textbox>
                                 </span><br /><br />
                                 ...
                                 <span style="font-size:medium;float:right;margin-right:5px" >
                                    <asp:RadioButtonList ID="rdoProjectedLeedSilver" runat="server"  RepeatDirection="Horizontal" onclick="handlerdoProjectedLeedSilverClick(this);">
                                   <asp:ListItem Value="Yes">Yes</asp:ListItem>
                                   <asp:ListItem Value="No">No</asp:ListItem>
                                   <asp:ListItem Value="Unknown">Unknown</asp:ListItem>
                               </asp:RadioButtonList>  
                                ...
                                <br /><br />
                                   ...
                                   <%-- blah--%>
         <span style="float:left;vertical-align:auto;margin-left:5px;font-size:large">What level is the building currently projecting to achieve?</span>
      <span style="float:right;margin-bottom:40px;width:120PX">  
    <asp:DropDownList ID="ddCurrProjected" Width="105px"  runat="server" >
              <asp:ListItem Value="0"> Please select </asp:ListItem>
     ...
     <br /><br /><br />
                                 ...
                                 <span id="rdoleedwaiverspan" style="font-size:medium;float:right;vertical-align:top;margin-right:5px"   >
                                 ...
                                 <span style="font-size:medium;float:right;margin-right:5px" >
                                         <asp:RadioButtonList ID="rdoCertStatus" runat="server"  RepeatDirection="Horizontal"  onclick="handleashraecert(this);" >
 ...
 <span style="float:right;margin-bottom:40px;width:0PX">
   ...
   &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<asp:DropDownList ID="ddLEEDCertLevelAcheived" Width="105px"  runat="server" >
             ...
             <asp:ListItem Value="Platinum">Platinum</asp:ListItem>
                             ...
                             <div style="background-color:#e6ecff">
                               <asp:Panel ID="PnlNetZero" BorderStyle="Solid" runat="server">
                                 ...
                                 <span style="font-size:medium;float:right;margin-right:5px" >
                                     <asp:RadioButtonList ID="rdoNetZApplicable" runat="server"  RepeatDirection="Horizontal"  onclick=" handlenetzerorequirement();" >
                                            <asp:ListItem Value="Yes">Yes</asp:ListItem>
                                            <asp:ListItem Value="No">No</asp:ListItem>
                                            <asp:ListItem Value="Unknown">Unknown</asp:ListItem>
                                        </asp:RadioButtonList> 
     ...
     <span style=" font-size:large;float:left;margin-left:20px;">Applicable version of ASHRAE 90.1<br />See VA Sustainable Design manual and/10 CFR 433.100</span>
      ...
      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style=" font-size:large;float:left;margin-left:20px;">Is the building required to perform at least 30% better than the<br /> applicable version as ASHRAE 90.1 as required by 10 CFR 433.100</span><br /><br />
       ...
       <span style="font-size:medium;float:right;margin-right:5px;margin-top:2px" >
          <asp:RadioButtonList ID="rdoASHRAE30PerCent" runat="server"  RepeatDirection="Horizontal" onclick="handleperform30PCBetter(event,this);" >
             <asp:ListItem Value="Yes">Yes</asp:ListItem>
             <asp:ListItem Value="No">No</asp:ListItem>
             <asp:ListItem Value="Unknown">Unknown</asp:ListItem>
         </asp:RadioButtonList>
    ...
    <br /><br /><br />
     ...
     &nbsp;<span style=" font-size:large;float:left;margin-left:20px;">If the building will not perform 30% better than the applicable<br /> version as ASHRAE 90.1, please enter an explanation.</span><br />
          <span style="float:right;vertical-align:middle;margin-right:5px">
      ...
      <span style="float:right;vertical-align:auto;margin-right:5px;margin-bottom:40px;">
          ...
          <asp:textbox ID="txtExceedsBaseline" Width="30px" runat="server"  MaxLength="2"></asp:textbox>%
      ...
      <span style="font-size:medium;float:right;margin-right:5px;margin-top:2px" >
          <asp:RadioButtonList ID="rdoCostEffective" runat="server"  RepeatDirection="Horizontal" onclick="handlecosteffective(event,this); "  >
             <asp:ListItem Value="Yes">Yes</asp:ListItem>
             <asp:ListItem Value="No">No</asp:ListItem>
	         <asp:ListItem Value="Unknown">Unknown</asp:ListItem>																
         </asp:RadioButtonList>
       ...
       <span style="font-size:large;float:left;margin-left:20px;">If energy efficiency is NOT determined to be lifecycle cost-effective please provide an explanation below.</span>
         <span style="float:right;vertical-align:middle;margin-right:5px">
            <asp:textbox  ID="txtDesignNotCostEffective" Width="300px" TextMode="MultiLine"  Height="80px" runat="server"></asp:textbox>
         </span><br /><br /><br /><br /><br />
         <br /><br /><br />

      <span style="float:left;width:800px;vertical-align:auto;font-size:large;margin-left:20px">Check here if final design<br /></span>
